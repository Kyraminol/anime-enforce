...
def main():
    ...
    for item in items:
        for section, episodes in item.episodes.items():
            indexes = input(f"{section.capitalize()} | Episodes: {len(episodes)}\n"
                            f"Input 0 for all, input -1 for last one or episodes number"
                            f" to download, comma separated (e.g.: 1, 2, 4): ").split(",")
            print(indexes)
            if "0" in indexes:
                indexes = ",".join([str(x) for x in range(1, len(episodes) + 1)])
            if "-1" in indexes:
                indexes = "0"
            episodes = [episodes[int(x) - 1] for x in indexes]
            for episode in episodes:
                outtmpl = f"downloads/{sanitize_filename(item.name)}/%(title)s.%(ext)s"
